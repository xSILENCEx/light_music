...
class QQMusicHelper {
  ...
  static Future<dynamic> getRecommendQQMusic({bool isTop = false}) async {
    ...
    if (_qqListData.isNotEmpty) {
      _qqListData.removeWhere((m) => m['data']['pay']['payplay'] == 1);

      ///从原始数据中解析出数据并加入播放列表
      return Stream.fromIterable(_qqListData)
          .asyncMap((m) async => LpMusic(
                m['data']['songmid'],
                m['data'],
                MusicType.follow,
                m['data']['songname'],
                ...
                m['data']['albumname'],
                m['data']['interval'],
                await _getQQPlayUrl(m['data']['songmid']),
  ...
  static Future<dynamic> searchQQMusic(String keyWord) async {
    ...
    if (_qqListData.isNotEmpty) {
      _qqListData.removeWhere((m) => m['pay']['payplay'] == 1);
  ...
  ///获取qq音乐的播放地址
  static Future<String> _getQQPlayUrl(String songmid) async {
