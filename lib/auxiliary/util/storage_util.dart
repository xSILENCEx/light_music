...
import 'package:light_player/auxiliary/others/data_format.dart';
...
class Storage {
  ...
  static getToday() {
    ...
    final String today = formatDate(now, [yyyy, '-', mm, '-', dd]);
    return today;
  }
  ...
  static getYesDay() {
    final DateTime now = DateTime.now();
    final yes = new DateTime(now.year, now.month, now.day - 1);
    final String yesterday = formatDate(yes, [yyyy, '-', mm, '-', dd]);
    return yesterday;
  }
  ...
  static getBYDay() {
    final DateTime now = DateTime.now();
    final bYes = new DateTime(now.year, now.month, now.day - 2);
    final String bYesterday = formatDate(bYes, [yyyy, '-', mm, '-', dd]);
    return bYesterday;
  }
  ...
  static getNowTime() {
    ...
    final String now = formatDate(dateTime, [mm, '.', dd, ' ', hh, ':', nn]);
    return now;
  }
}
